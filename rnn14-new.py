...
class MetaRNN(BaseEstimator):
    def __init__(self, n_in=5, n_hidden=50, n_out=5, learning_rate=0.01,
                 n_epochs=100, L1_reg=0.00, L2_reg=0.00002, learning_rate_decay=1,
...
##############  Add another Test  ####################
    ...
    file_path4 = os.path.join(BASE_DIR, '../RNN-data/testdata/inputerror-b14-60-50-15-y.txt')
...
###########  End add another Test  ##############
    ...
    count = 0
    dataError = []
    file_path3 = os.path.join(BASE_DIR, '../RNN-data/traindata/inputerror-b14-60-45-y.txt')
    ...
    fmatrix = file('../RNN-data/matrix/freqMatrix-b14.txt','a+')
    for i in range (lineNum):
    ...
    ferror1 = file('errorRate/errorRate-b14-no.txt','a+')
    ferror2 = file('errorRate/errorRate-b14-single.txt','a+')
    [seqNum,lineNum,colNum] = targetsTest1.shape
